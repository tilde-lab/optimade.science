...
const search = derived<[Writable<StringParams>, Readable<Param[]>], StructuresByProviders>(
    ...
    ([$query, $selectedProviders], set) => {
        ...
        providers
            .get()
            .then(() => getStructuresAll($selectedProviders, $query.params.filter, $query.params.page, $query.params.limit, false))
            .then((results = []) => {
                set(results);
            });
    },
...
export const searchAll = asyncable<[Readable<StructuresByProviders>], StructuresByProviders>(($search) => Promise.all($search), null, [search]);
...
export const getTotal = asyncable<[Readable<StructuresByProviders>], number>(
    async ($search) => {
        ...
        const returnedTotals = filteredProviders.reduce<number[]>((acc: number[], [apis, _provider]: [Types.StructuresResponse[], Types.Provider]) => {
...
export const total = derived<[Writable<StringParams>, Readable<Param[]>, Asyncable<number>], number>(
    ...
    ([$query, $selectedProviders, $getTotal], set) => {
        ...
        else if (page === $query.params.page || $selectedProviders)
