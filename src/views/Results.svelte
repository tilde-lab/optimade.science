	...
	{#each $results as result, index}
		{#await result}
			<Section heading="Searching...">
				<Loader.Cards backgroundColor="#f3f3f3" foregroundColor="#ecebeb" height={290} rows={2} w={125} h={130} {width} {cols} />
			</Section>
		{:then [apis, provider]}
					...
					<Grid stack>
						{#each apis[0].data as item, i}
							<Col col="2" md="3" sm="4" xs="6">
								<Modal open={$fragment === `#${provider.id}-${item.id}`} on:toggle={clearFragmentOnClose}>
									<a
									...
									>
										<Card style="min-height: 130px; text-align: center;">
...
<script lang="ts">
	...
	function delay(index: number, i: number, m = 100) {
		switch (true) {
		...
		return index * m * 10 + Math.trunc(i / cols) * cols * m + Math.trunc(i % cols) * m;
	}
