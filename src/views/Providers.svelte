			...
			{#each items as item}
				<Col col={$media.sm ? '' : 1}>
					<Popover side="bottom">
						...
						<Card slot="content">
							<a slot="subtitle" href={item.attributes.homepage || ''} target="_blank"
								><img class="provider-logo" src={providerLogos[item.id] || OPT} alt={item.attributes.name} /></a
							>
							<div slot="title" class="h6 text-center my-2">
								<a href={item.attributes.homepage || ''} target="_blank">{item.attributes.name}</a>
							</div>
							<div class="text-tiny text-center">
								{item.attributes.description}<br />Base URL: <a href={item.attributes.base_url} target="_blank">{item.attributes.base_url}</a>
							</div>
						</Card>
					</Popover>
				</Col>
...
<AddProviderModal bind:open={modalOpen} />
...
<script lang="ts" context="module">
	...
	import AddProviderModal from '@/views/AddProviderModal.svelte';
