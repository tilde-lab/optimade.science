	...
	<Grid>
		<Col md="12">
			{#if data}
				...
				<small class="form-text text-muted">filter object by field name</small>
		...
		<Col md="12">
			...
			<br />
			<div class="module">
				<iframe name="visualisationFrame" bind:this={iframe} src={module} frameborder=0 scrolling="no" />
			</div>
		</Col>
	</Grid>
</div>
...
<script lang="ts">
	...
	onMount(() => {
		window.addEventListener('message', (message) => {
			if (message.data === 'need-absolidix-login'){
				console.log(message.data);
			} else if (message.data === 'absolidix-ok'){
				console.log(message.data);
			}
		});
	});
